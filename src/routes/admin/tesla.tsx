...
import { adminListOrders, adminListHoldings, adminUpdateQuote, getQuote } from "@/lib/m5";
import { toast } from "sonner";

export const Route = createFileRoute("/admin/tesla")({
  component: AdminTesla,
});
...
function AdminTesla() {
  ...
  const updatePrice = async () => {
    ...
    if (!p || p <= 0) return;
    try { await adminUpdateQuote("TSLA", { price: p, previous_close: quote?.price ?? p }); toast.success("Quote updated"); reload(); }
    catch (e: any) { toast.error(e.message); }
  };

  return (
    <div>
      <PageHeader eyebrow="MARKETS" title="Tesla Stock Operations" subtitle="Manage TSLA quotes, orders, and investor positions." />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-6">
        <StatCard label="TSLA Quote" value={`$${Number(quote?.price ?? 0).toFixed(2)}`} icon={<TrendingUp className="h-4 w-4" />} />
      ...
      <Panel title="Quote Control" className="mb-6">
        <div className="flex gap-3 items-end max-w-md">
          <div className="flex-1">
            <label className="text-xs text-muted-foreground">TSLA Price</label>
